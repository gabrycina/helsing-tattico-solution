...
class UnitNavigator:
    ...
    def get_navigation_impulse(self) -> simulation_pb2.Vector2:
        ...
        if distance < self.arrival_threshold:
            ...
            logger.debug(
                f"Arrived at target. Applying breaking force: ({breaking_x:.2f}, {breaking_y:.2f})"
            )
        ...
        proximity_factor = max(
            0, 1.0 - (distance / 15.0)
        )  # Adjusted distance scale for larger movements
        ...
        logger.debug(
            f"Navigation: dist={distance:.2f}, impulse=({impulse_x:.2f}, {impulse_y:.2f})"
        )
