...
TOKEN = "".join(
    os.urandom(40).hex()
)  # Generate 40 random bytes and convert to hex string
...
def control_sensor_unit(simulation_id, unit_id):
    ...
    unit_logger.info(
        f"Initializing sensor unit {unit_id} for simulation {simulation_id}"
    )
    ...
    for response in responses:
        unit_logger.debug(
            f"Unit {unit_id} position: ({response.pos.x}, {response.pos.y})"
        )
        ...
        if response.HasField("detections"):
            ...
            for direction in [
                ...
                if detections.HasField(direction):
                    ...
                    unit_logger.debug(
                        f"Unit {unit_id} detected {cls_name} {direction.upper()} at {detection.distance}"
                    )
...
def launch_strike_unit(simulation_id, base_pos):
    ...
    with grpc.insecure_channel(SERVER_ADDRESS) as channel:
        ...
        logger.info(
            f"Strike unit {response.id} launched at position ({response.pos.x}, {response.pos.y})"
        )
        logger.info(
            f"Distance from base: {distance_from_base:.2f} units, Offset: ({dx:.2f}, {dy:.2f})"
        )
...
def control_strike_unit(simulation_id, unit_id):
    ...
    unit_logger.info(
        f"Initializing strike unit {unit_id} for simulation {simulation_id}"
    )
    ...
    # Shared state between response handler and command generator
    response_queue = Queue()
    strike_state = {"has_target_info": False, "target_direction": None, "counter": 0}
        ...
        while True:
            ...
            try:
                ...
                for message in response.messages:
                    ...
                    if "TARGET_DETECTED" in msg_str:
                        ...
                        if len(parts) >= 2:
                            ...
                            unit_logger.info(
                                f"Strike unit received target info: direction={parts[1]}"
                            )
                ...
                if strike_state["has_target_info"]:
                    ...
                    unit_logger.info(
                        f"Strike unit moving toward target in {strike_state['target_direction']}"
                    )
                else:
                    # No target info - just make a simple circular motion
                    directions = [(0.5, 0.5), (-0.5, 0.5), (-0.5, -0.5), (0.5, -0.5)]
                    idx = strike_state["counter"] % 4
                    vector = simulation_pb2.Vector2(
                        x=directions[idx][0], y=directions[idx][1]
                    )
                    unit_logger.info(
                        f"Strike unit searching: direction={idx}, vector=({directions[idx][0]}, {directions[idx][1]})"
                    )
                ...
                unit_logger.error(
                    "Strike unit queue.Empty exception - should not happen with blocking get()"
                )
                pass
...
if __name__ == "__main__":
    ...
    logger.info(
        f"Base position: ({sim_response.base_pos.x}, {sim_response.base_pos.y})"
    )
    ...
    for sensor_id in sim_response.sensor_units.keys():
        sensor_thread = threading.Thread(
            target=control_sensor_unit, args=(simulation_id, sensor_id), daemon=True
        )
